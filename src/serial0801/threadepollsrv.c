...
int brake_rolling_counter = 0;
// int speed_rolling_counter = 0;
                        ...
                        *p_gps = connfd;
                        ...
                        if(pm->tfd == packet302fd) {
                            ...
                            if(is_file_descriptor(packet302fd)) {
                                ...
                                // send302packet.data[6] =
                                //         (send302packet.data[6] & TAKELOW4BIT) 
                                //         | (speed_rolling_counter << 4);
                                // ++speed_rolling_counter;
                                // if(speed_rolling_counter > 15)
                                //     speed_rolling_counter = 0;
                                // check_sums(send302packet.data, 7);  
                                // 302的报文加校验和和循环计数，都不能正常工作
                            ...
                            // debug("epoll packet132fd ");
                            //线程死了对应的定时器也会被清除
                            if(is_file_descriptor(packet132fd)) {
                                ...
                                if(write_packet(chassis_conn_fd, &send132packet) < 0) {
                                    delfd4epoll(epollfd, packet132fd);
                                    // thread_cancel(&tid_vehicle);
                                }
                                // printf_packet(&send132packet);
                                // debug("\n");
                            }
    ...
    while ((opt = getopt(argc, argv, "s:t:c")) != -1) {
        switch (opt) {
            ...
            case 't':  //使用TCP-IP收发chassis的can数据  
                ...
                break;
            case 'c':  //使用CAN端口直接收发chassis的can数据 
