...
int write_serial_packet(const int conn, can_frame_packet_t *sendframe)
{
    ...
    int res = write(conn, write_buffer, 11);  //only write 11 bytes
    if(res < 0) {
        debug("write failed ");
        debug("should delete timers!\n");
        return -1;
...
{
    unsigned char read_buffer[11] = {0};
    int read_len = 0;
    int length = 0;
    unsigned char msg[11] = {0};
    tcflush(conn, TCIFLUSH);    //clear serail read buffer
    fflush(stdin);          //stdin flush
    //堵塞地读数据
    while((read_len = read(conn, read_buffer, sizeof(read_buffer) - 1)) > 0) {
        printf("read_buffer:%d ", read_len);
        if(read_len == 1 && read_buffer[0] == 0x00)
            continue;       //for other ctrl+c
        else if(read_len < 0)
            printf("read error\n");
        ...
        printf("\n");
        memcpy(msg+length, read_buffer, read_len);
        length += read_len;
        if(length >= 10) {
            break;
        }
